...
def generate_default_schedule(usr: str, uhaveid: int):
    ...
    # remove duplicates
    schedule_target_no_duplicates, schedule_pid_no_duplicates = [], []
    for t, pid in zip(schedule_target, schedule_pid):
...
# Y get the time needed to observe of each target in projects
...
def get_observable_time(uhaveid: int, schedule_pid: list, sorted_target: list):
    ...
    # convert basetime from local to UTC time
    ...
    for pid, tar in zip(schedule_pid, sorted_target):
        ...
        # skip the target if its t2o is 0
        if t2o_total == 0:
            continue
        ...
        if str(t_start) != 'nan' and str(t_end) != 'nan':
            ...
            # add datetime format to each observable target
            temp = {}
            temp['TID'] = tid
            temp['Start'] = t_start
...
def calculate_default_schedule(base_time, tid_list, observable_time, hint_msgs):
    ...
    for i in range(1440):
        ...
        # consider the target to the end of the list
